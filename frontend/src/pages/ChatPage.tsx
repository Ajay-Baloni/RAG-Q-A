...
import type { Citation, ConversationSummary, Message, RetrievedSource } from '../types';
...
export function ChatPage() {
  ...
  const state = location.state as { documentIds?: string[]; skipLoad?: boolean } | null;
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      ...
      await streamQuestion(targetId, q, {
        onSources: (sources) =>
          patch(assistantId, (m) => ({ ...m, citations: sourcesToCitations(sources) })),
        onToken: (text) => patch(assistantId, (m) => ({ ...m, content: m.content + text })),
        onDone: (res) => {
          patch(assistantId, (m) => ({ ...m, id: res.messageId, modelUsed: res.model }));
          refreshConversations();
        },
  ...
  return (
    <div className="grid h-full grid-cols-1 gap-5 md:grid-cols-[230px_1fr]">
      ...
      <div className="flex h-full flex-col overflow-hidden rounded-2xl border border-line bg-surface/40">
              ...
              <div className="grid h-full place-items-center text-center">
                <div className="max-w-xs">
                  ...
                  <p className="mt-1 text-sm text-muted">
                    Ask a question about your selected documents — answers stream in with citations.
                  </p>
                </div>
              </div>
            )}
            {messages.map((m) => {
              const activeOrder = hovered?.messageId === m.id ? hovered.order : null;
              const onHover = (order: number | null) =>
                setHovered(order === null ? null : { messageId: m.id, order });
              return (
                <div key={m.id} className="space-y-1.5">
                  <MessageBubble message={m} activeOrder={activeOrder} onHoverCitation={onHover} />
                  ...
                  {m.role === 'ASSISTANT' && m.citations && m.citations.length > 0 && (
                    <CitationList citations={m.citations} activeOrder={activeOrder} onHover={onHover} />
                  )}
                </div>
              );
            })}
        ...
        {error && <p className="px-5 py-2 text-sm text-red-600 dark:text-red-400">{error}</p>}
